...
Printrun. If not, see <http://www.gnu.org/licenses/>."""
    ...
    def _add_settings(self, size):
        ...
        self.settings._add(StringSetting("gcview_color_travel", "#99999900", _("3D view travel moves color"), _("Color of travel moves in 3D view"), "Colors"), self.update_gcview_colors, validate = check_rgba_color)
        ...
        self.settings._add(StaticTextSetting("note1", _("Note:"), _("Changing some of these settings might require restarting the program"), group = "UI"))
    ...
    def sentcb(self, line, gline):
        ...
        if not gline:
            ...
            if gline_s is not None:
                ...
                ## NEED CHANGES HERE
                if self.display_gauges: wx.CallAfter(self.hottgauge0.SetTarget, temp)
    ...
    def update_tempdisplay(self):
        try:
            ...
            if hotend_temp1 is not None and self.settings.extruders == 2:
                if self.display_graph: wx.CallAfter(self.graph.SetExtruder1Temperature, hotend_temp1)
                if self.display_gauges:wx.CallAfter(self.hottgauge1.SetValue, hotend_temp1)
